...
const CompensatoryTimeBalanceScreen = ({ navigation }: IProps) => {
  ...
  const {
    ...
    isFetching,
    data: periods,
  } = useQuery([QueryKeys.COMPENSATORY_TIME, user.employeeId], () => GetCompensatoryTime(user.employeeId));

  const renderItem = ({ item }: any) => {
    return (
      <Box mx={5} py={3} borderBottomWidth="1" borderColor={'gray.400'} justifyContent="center">
        <VStack w="100%">
    ...
    <Box safeArea backgroundColor="#fff" flex={1}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<NoData message="No hay tiempo compensatorio por mostrar" />}
          ListFooterComponent={
            <Box mx={5} py={3}>
              <HStack space={3} justifyContent="space-between" mb={1} mt={1}>
                <VStack w="60%">
                ...
                <VStack w="40%" alignItems="flex-start">
                  ...
                  >
                    {decimalAHora(periods.reduce((acc, curr) => acc + curr.saldoHoras, 0))}
                  </Text>
                </VStack>
              </HStack>
            </Box>
