...
import { Config } from 'reanimated-collapsible-helpers/lib/typescript/src/types';
...
const CollapsibleView = ({
  ...
  }, [expanded, handleArrowRotate, show]);
      ...
      >
        <View
          ...
          style={{
            ...
            alignItems: 'center',
            ...titleStyle,
            marginLeft: nested === 1 && Platform.OS === 'web' ? '12px' : 34,
          }}
          {...titleProps}
        >
          <View
            // eslint-disable-next-line react-native/no-inline-styles
            style={{
              height: state === 'expanded' ? (Platform.OS === 'web' ? 0 : '160%') : '70%',
              marginBottom: 12,
              borderLeftColor: '#d9d9d9',
              borderLeftWidth: nested === 1 ? 2 : 2,
            }}
          />
          {nested > 1 && lastCommentIndex > -1 && parentCommentLength === 1 && !isParentLast ? (
            <Box height="100%" width={1} style={styles.nestedTwoLine} />
          ) : null}

          {nested === 2 && parentCommentLength === length - 1 && length > 1 && !isParentLast ? (
            <Box height="100%" width={1} style={styles.nestedTwoLine} />
          ) : null}
          ...
          {state !== 'expanded' ? <Box height={0.5} width={10} style={styles.horizontalLine} /> : null}
          ...
          <Text style={styles.title}>{state === 'collapsed' ? title : collapsedTitle}</Text>
        </View>
      </TouchableComponent>
      {/* {mounted ? ( */}
      <AnimatedSection
