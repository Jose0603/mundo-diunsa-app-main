...
export const ChatMessage = (props: ChatMessageProps): React.ReactElement => {
  ...
  const renderContentElement = (): React.ReactElement => {
    return children(message, {
      container: [message.isSender ? styles.contentOut : styles.contentIn],
    });
  };

  const renderIndicator = (): React.ReactElement => (
    <ChatMessageIndicator
      style={[message.isSender ? styles.indicatorOut : styles.indicatorIn, styles.indicator]}
      reverse={message.isSender}
    />
  );

  return (
    <View {...viewProps} style={[message.isSender ? styles.containerOut : styles.containerIn, styles.container, style]}>
