...
import CollapsibleView from './CommentsComponents/CollapsibleView';
        ...
        <>
          ...
          {item?.child_comments?.length > 0 ? (
            <CollapsibleView
              ...
              collapsedTitle={item?.child_comments?.length === 1 ? 'hide reply' : 'hide replies'}
                ...
                return (
                  <React.Fragment key={levelOneComment.id}>
                    <Comment
                      ...
                      lastCommentGrandId={getLastChildCommentId(item)}
                      nested={1}
                      isParentLast={levelOneComment.id === item.child_comments[item.child_comments.length - 1].id}
                      isParent={levelOneComment?.child_comments?.length > 0}
                      parentCommentLength={item?.child_comments?.length || 0}
                      totalChildren={levelOneComment?.child_comments?.length || 0}
                      isLast={levelOneComment.id === item.child_comments[item.child_comments.length - 1].id}
                    />
                    {levelOneComment?.child_comments?.length > 0 ? (
                      <CollapsibleView
                        ...
                        collapsedTitle={levelOneComment?.child_comments?.length === 1 ? 'hide reply' : 'hide replies'}
                        isParentLast={levelOneComment.id === item.child_comments[item.child_comments.length - 1].id}
                        parentCommentLength={levelOneComment.child_comments.length}
                        titleStyle={styles.showMoreCommentsTwo}
                        unmountOnCollapse
                      >
                        {levelOneComment.child_comments.map((levelTwoComment: any, idxTwo: number) => {
                          return (
                            <React.Fragment key={levelTwoComment.id}>
                              <Comment
                                ...
                                isLast={
                                  levelTwoComment.id ===
                                  levelOneComment.child_comments[levelOneComment.child_comments.length - 1].id
                                }
                                isParentLast={
                                  levelOneComment.id === item.child_comments[item.child_comments.length - 1].id
                                }
                                totalChildren={levelOneComment?.child_comments?.length || 0}
                              />
                            </React.Fragment>
